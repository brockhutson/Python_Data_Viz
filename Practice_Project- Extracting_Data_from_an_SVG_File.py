# -*- coding: utf-8 -*-
"""
Created on Wed Sep 11 13:58:22 2019
...
-Parse the XML in the SVG file using an existing Python module and 
 extract the \verb|d |d and \verb|id|id attributes for each county.
-Extract the county's boundary coordinates (as a list of map coordinates) 
 from the path data for each county,
-Compute the center of each county using our provided code and 
 output a CSV file with FIPS codes and county centers.
"""
...
# Parse the XML in USA SVG file extract county attributes
...
def get_county_attributes(svg_file_name):
    ...
    for path in doc.getElementsByTagName('path'):
        ...
        county_boundaries = path.getAttribute('d')
        county_attribute_list.append((fips_code, county_boundaries))
    doc.unlink()
...
def test_boundary_code():
    ...
    print('=========Boundary Coordinates 1===========')
    print(boundary_coords1)
    print('=========Boundary Coordinates 2===========')
    print(boundary_coords2)
    print('=========County Center Coords 1===========')
    print(compute_county_center(boundary_coords1))
    print('=========County Center Coords 2===========')
...
def process_county_attributes(svg_file_name, csv_file_name):
    ...
    county_attributes = get_county_attributes(svg_file_name)
    
    # Ouput CSV file
    with open(csv_file_name, 'w', newline='') as csv_file:
        csv_writer = csv.writer(csv_file, delimiter=',', quoting=csv.QUOTE_MINIMAL)
        for (fips, boundary) in county_attributes:
